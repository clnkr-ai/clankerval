...
func validateTaskJSON(path string, raw taskJSON) (Task, error) {
	...
	if err := validateTaskRelativePath(taskRoot, path, "instruction_file", instructionFile, false); err != nil {
		return Task{}, err
	}
	...
	if raw.SeedTranscriptFile != nil {
		...
		if err := validateTaskRelativePath(taskRoot, path, "seed_transcript_file", seedTranscriptFile, false); err != nil {
			return Task{}, err
		}
	...
	if raw.ScriptedTurnsFile != nil {
		...
		if err := validateTaskRelativePath(taskRoot, path, "scripted_turns_file", scriptedTurnsFile, false); err != nil {
			return Task{}, err
		}
...
func validateTaskRelativePath(taskRoot, path, field, value string, allowDot bool) error {
	if filepath.IsAbs(value) {
		return fmt.Errorf("%s: field %q must stay within task root %q, got %q", path, field, taskRoot, value)
	}
